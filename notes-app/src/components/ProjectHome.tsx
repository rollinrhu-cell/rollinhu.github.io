      ...
      <div className="space-y-6">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Sources</h3>
            <button
              onClick={onAddSource}
          ...
          {sources.length === 0 ? (
            ...
            <div className="space-y-2">
              {sources.map(source => (
                <button
                  key={source.id}
                  className="w-full text-left p-3 bg-white border border-gray-200 rounded-lg hover:border-indigo-200 hover:bg-indigo-50/30 transition-colors group"
                  onClick={() => onSelectSource(source.id)}
                >
                  <div className="flex items-start gap-2">
                    <span className="text-xs uppercase font-medium px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 flex-shrink-0 mt-0.5">
                      {source.type}
                    </span>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Themes</h3>
            <button
              onClick={onAddTheme}
              className="text-xs text-indigo-600 hover:text-indigo-700 font-medium"
            >
          ...
          {themes.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              {themes.map(theme => (
                <button
                  key={theme.id}
                  className="text-left p-3 bg-white border border-gray-200 rounded-lg hover:border-indigo-200 hover:bg-indigo-50/30 transition-colors"
        ...
        {/* Quick actions */}
        <section className="pt-2 border-t border-gray-100 flex gap-3">
          <button
